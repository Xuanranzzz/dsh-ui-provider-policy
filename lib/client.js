	...
	factory: (require) => {
		...
		* default whitelist — quota-shaped 429s only retry when explicitly listed. */
		...
		const en = {
			...
			strategyStandardHint: "Retry count is editable; every provider keeps its own retryable codes and backoff.",
			strategyAggressiveHint: "Retry count is locked at 1000; waits become 1s, 2s, 4s, then a 5s cap, with jitter off.",
			...
			previewParamsStandard: "Standard leaves every provider's backoff untouched; the curve reads the first provider's current values — initial {initial}, cap {max}, ×2, jitter ±{percent}%",
			previewParamsAggressive: "Aggressive writes this backoff into every provider — initial {initial}, cap {max}, ×2, no jitter",
		...
		const zh = {
			...
			strategyStandardHint: "重试次数可调;不改动各供应商已有的可重试错误码与退避参数。",
			strategyAggressiveHint: "重试次数锁定为 1000,不可调整;退避固定为 1s、2s、4s,之后封顶 5s,并关闭随机抖动。",
			retryCount: "重试次数",
			...
			previewParamsStandard: "标准策略不改动各供应商的退避,曲线按第一个供应商的当前值绘制:初始 {initial} · 上限 {max} · ×2 · 抖动 ±{percent}%",
			previewParamsAggressive: "激进策略会把该退避写入所有供应商:初始 {initial} · 上限 {max} · ×2 · 无抖动",
			axisRetries: "重试次数",
			axisTime: "等待时间",
		...
		* @param props - { count, initialDelayMs, maxDelayMs, jitterRatio, aggressive, t }.
		*/
		function RetryPreview(props) {
			const { count, initialDelayMs, maxDelayMs, jitterRatio, aggressive, t } = props;
			...
			const width = 680;
			const height = 184;
			const pad = { left: 66, right: 24, top: 24, bottom: 34 };
			...
			for (const [index, tick] of ticks.entries()) {
				...
				chart.push(h("line", { key: "grid" + index, x1: pad.left, y1: y, x2: pad.left + plotWidth, y2: y, stroke: gridColor, strokeWidth: 1, strokeDasharray: tick === 0 ? void 0 : "2 5" }));
			...
			for (const index of xTicks) chart.push(h("text", { key: "xlabel" + index, x: xOf(index), y: height - 18, textAnchor: "middle", fill: labelColor, fontSize: 11 }, String(index + 1)));
			chart.push(h("line", { key: "xaxis", x1: pad.left, y1: baseline, x2: pad.left + plotWidth, y2: baseline, stroke: gridColor, strokeWidth: 1 }));
			chart.push(h("path", { key: "area", d: areaPath, fill: accent, fillOpacity: 0.07 }));
			chart.push(h("polyline", { key: "curve", points: line, fill: "none", stroke: accent, strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round" }));
			if (capped <= count) {
				chart.push(h("line", { key: "cap", x1: capX, y1: pad.top, x2: capX, y2: baseline, stroke: "var(--dsw-alias-border-l3)", strokeWidth: 1, strokeDasharray: "3 4" }));
				chart.push(h("circle", { key: "capdot", cx: capX, cy: yOf(waits[capped - 1]), r: 2.5, fill: labelColor }));
				chart.push(h("text", { key: "caplabel", x: capLabelX, y: pad.top + 12, fill: labelColor, fontSize: 11 }, t("previewCapFrom", { n: capped, d: formatDuration(maxDelayMs) })));
			}
			const totalText = t("previewTotal", { count, total: formatDuration(total) });
			const paramsText = aggressive === true
				? t("previewParamsAggressive", { initial: formatDuration(initialDelayMs), max: formatDuration(maxDelayMs) })
				: t("previewParamsStandard", { initial: formatDuration(initialDelayMs), max: formatDuration(maxDelayMs), percent: trimDecimal(jitterRatio * 100, 1) });
			return (0, react_jsx_runtime.jsxs)("div", { style: fieldStyle, children: [
				(0, react_jsx_runtime.jsx)("span", { style: fieldLabelStyle, children: t("retryPreview") }),
				h("svg", {
				...
				h("span", { style: fieldHintStyle }, totalText),
				h("span", { style: fieldHintStyle }, paramsText)
			] });
		}
			...
			* the aggressive strategy overwrites the backoff with its own preset. */
			...
			return (0, react_jsx_runtime.jsxs)("div", { style: cardStyle, children: [
				...
				(0, react_jsx_runtime.jsxs)("div", { style: fieldStyle, children: [
					...
					(0, react_jsx_runtime.jsx)("span", { style: fieldHintStyle, children: aggressive ? t("strategyAggressiveHint") : t("strategyStandardHint") })
				] }),
				(0, react_jsx_runtime.jsxs)("div", { style: fieldStyle, children: [
